function accuracy_rate = bucket_prediction(data, threshold, num_bins, prob_spam, prob_nonspam, bucket_thresh, bucket_prob_spam, bucket_prob_nonspam )
    [n,m] = size(data); 
    y = data(:,m); 
...
%              denominator = denominator * bucket_prob_nonspam(q,j);
            ...
            temp = log(bucket_prob_spam(q,j))-log(bucket_prob_nonspam(q,j)); 
             
         log_prob = log_prob + temp ; 
         end
%          % i did not smoothing 
%          if numerator == 0 
%              
